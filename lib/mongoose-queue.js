...
class MongooseQueue
{
    ...
    length(type, cb)
    {
        ...
        // TODO: maybe add helper function that returns the proper query so we don't have to redefine the query here
        // and in the methods above
        switch(type){
            // total count
            case 'all':
                query = {};
                break;
            // completed, error'd, or reached maxRetries
            case 'done': 
                query = {
                    $or: [
                        {done: true},
